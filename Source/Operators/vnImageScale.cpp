...
VN_STATUS vnBresenhamScaleLine(UINT8* pSrcBuffer, UINT32 uiSrcLength, UINT32 uiSrcStride,
                               UINT32 uiPixelPitch, UINT8* pDestBuffer, UINT32 uiDestLength,
                               UINT32 uiDestStride) {
	if (VN_PARAM_CHECK) {
...
VN_STATUS vnBresenhamScaleImage(CONST CVImage& pSrcImage, CVImage* pDestImage) {
	if (VN_PARAM_CHECK) { if (!pDestImage) { return vnPostError(VN_ERROR_INVALIDARG); } }
	...
	for (UINT32 y = 0; y < pSrcImage.QueryHeight(); y++) {
		...
		if (pSrcImage.QueryWidth() == tempImage.QueryWidth()) {
			...
			if (VN_FAILED(vnBresenhamScaleLine( pSrcData,
				...
				uiPixelPitch ))) { return vnPostError(VN_ERROR_EXECUTION_FAILURE); }
		}
	}
	...
	for (UINT32 x = 0; x < tempImage.QueryWidth(); x++) {
		...
		if (VN_FAILED(vnBresenhamScaleLine( pSrcData,
			...
			pDestImage->RowPitch() ))) { return vnPostError(VN_ERROR_EXECUTION_FAILURE); }
	}
