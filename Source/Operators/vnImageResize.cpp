...
VN_STATUS vnResizeImageNonSeparable(CONST CVImage& pSrcImage, VN_IMAGE_KERNEL_TYPE uiKernel,
                                    FLOAT32 fHRatio, FLOAT32 fVRatio, CVImage* pDestImage) {
		...
		for (UINT32 i = 0; i < pDestImage->QueryWidth(); i++) {
			...
			if (VN_FAILED(
				vnSampleImage( pSrcImage, uiKernel, VN_IMAGE_KERNEL_2D_COMBINED, fX, fY, fRadius,
					pOutputData ))) { return vnPostError(VN_ERROR_EXECUTION_FAILURE); }
		}

	return VN_SUCCESS;
}

VN_STATUS vnResizeImageSeparable(CONST CVImage& pSrcImage, VN_IMAGE_KERNEL_TYPE uiKernel,
                                 FLOAT32 fHRatio, FLOAT32 fVRatio, CVImage* pDestImage) {
	CVImage tempImage;

	if (VN_FAILED(
		vnCreateImage( pSrcImage.QueryFormat(), pDestImage->QueryWidth(), pSrcImage.QueryHeight(), &
			tempImage ))) { return vnPostError(VN_ERROR_EXECUTION_FAILURE); }
	...
	for (UINT32 j = 0; j < pSrcImage.QueryHeight(); j++)
		for (UINT32 i = 0; i < pDestImage->QueryWidth(); i++) {
			...
			if (VN_FAILED(
				vnSampleImage( pSrcImage, uiKernel, VN_IMAGE_KERNEL_1D_HORIZONTAL, fX, fY, fHRatio,
					pOutputData ))) { return vnPostError(VN_ERROR_EXECUTION_FAILURE); }
		}
...
VN_STATUS vnResizeImageWithPadding(CONST CVImage& pSrcImage, CVImage* pDestImage) {
	if (VN_PARAM_CHECK) {
		if (!VN_IS_IMAGE_VALID(pSrcImage) || !pDestImage) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
...
VN_STATUS vnResizeImage(CONST CVImage& pSrcImage,
	...
	FLOAT32 fVertRatio = (1 == uiHeight
		                      ? 1.0f
		                      : (FLOAT32)(pSrcImage.QueryHeight() - 1) / (uiHeight - 1));
...
VN_STATUS vnResizeImageNoAlloc(CONST CVImage& pSrcImage,
	...
	if (VN_PARAM_CHECK) {
		if (!VN_IS_IMAGE_VALID(pSrcImage) || !VN_IS_IMAGE_VALID(pDestImage)) {
			return vnPostError(VN_ERROR_INVALIDARG);
		}
	}

	CONST auto uiWidth = pDestImage.QueryWidth();
	CONST auto uiHeight = pDestImage.QueryHeight();
	...
	if (VN_IMAGE_RESIZE_POW2 & uiFlags && (uiWidth != vnAlign2(uiWidth) || uiHeight != vnAlign2(uiHeight))) {
		//
		// Check valid dest.
		//
		return vnPostError(VN_ERROR_INVALIDARG);
	...
	if (VN_IMAGE_RESIZE_SYMMETRIC & uiFlags && uiWidth != uiHeight) {
		//
	...
	if (uiWidth == pSrcImage.QueryWidth() && uiHeight == pSrcImage.QueryHeight()) {
		return vnCloneImageData(pSrcImage, pDestImage);
	}
