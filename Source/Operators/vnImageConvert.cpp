...
VN_STATUS vnConvertImage(CONST CVImage& pSrcImage, VN_IMAGE_FORMAT destFormat,
                         CVImage* pDestImage) {
	if (VN_PARAM_CHECK) {
	...
	if (pSrcImage.QueryFormat() == destFormat) { return vnCloneImage(pSrcImage, pDestImage); }
	...
	if (VN_FAILED(
		vnCreateImage( destFormat, pSrcImage.QueryWidth(), pSrcImage.QueryHeight(), pDestImage ))) {
