...
VN_STATUS vnUnpackFloatChannel(UINT8 uiChannelRate, UINT8* pRawChannel, FLOAT64* pOutChannel) {
	...
	if (VN_PARAM_CHECK) {
		if (0 == uiChannelRate || !pOutChannel) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
...
VN_STATUS vnUnpackFixedChannel(UINT8 uiChannelRate, BOOL bSigned, UINT64 uiRawData,
                               UINT64* pOutChannel) {
	//
	// This parameter check is exhaustive, since this function is static and will not presently
	// be called with unchecked parameters, but we redundantly validate the parameters in case 
	...
	if (bSigned) {
		...
		if (iRawData & ((INT64)1 << (uiChannelRate - 1))) { iRawData |= ~uiMaskChannelVal; }

		if (iRawData < 0) { (*pOutChannel) = (INT64)VN_MIN_INT32 * iRawData / iMinChannelVal; }
		else { (*pOutChannel) = (INT64)VN_MAX_INT32 * iRawData / iMaxChannelVal; }
	}
	...
	else { (*pOutChannel) = (INT64)VN_MAX_INT32 * uiRawData / uiMaxChannelVal; }
...
VN_STATUS vnUnpackFixedPixel(UINT8* pRawPixel, VN_IMAGE_FORMAT format, VN_PIXEL_BLOCK* pOutBlock) {
	...
	for (UINT32 i = 0; i < uiChannelCount; i++) {
		...
		if (VN_FAILED(
			vnUnpackFixedChannel( uiChannelRate, VN_IS_SIGNED_FORMAT( format ), uiRawData, (UINT64 *) &(
				pOutBlock->iChannelData[i] ) ))) { return vnPostError(VN_ERROR_EXECUTION_FAILURE); }
