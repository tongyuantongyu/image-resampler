...
VN_STATUS vnConvertBlock(CONST VN_PIXEL_BLOCK& pSrc, VN_IMAGE_FORMAT destFormat,
	...
	VN_IMAGE_PRECISION destPrecision = (VN_IS_FLOAT_FORMAT(destFormat))
	...
	if (pSrc.uiPrecision == destPrecision) {
		...
		if (VN_IMAGE_PRECISION_FLOAT == destPrecision) {
			...
			for (UINT8 i = 0; i < 4; i++) {
				...
				else { pDest->fChannelData[i] = (FLOAT64)pSrc.iChannelData[i] / VN_MAX_INT32; }
			...
			for (UINT8 i = 0; i < 4; i++) {
				...
				else { pDest->iChannelData[i] = (INT64)((FLOAT64)fTempChannels[i] * VN_MAX_INT32); }
			}
		}
	}
