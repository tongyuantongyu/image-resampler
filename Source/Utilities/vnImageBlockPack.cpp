...
VN_STATUS vnPackFloatChannel(CONST FLOAT64& pInChannel, UINT8 uiChannelRate, UINT8* pRawChannel) {
	...
	if (VN_PARAM_CHECK) {
		if (0 == uiChannelRate || !pRawChannel) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
	...
	case 16: *((FLOAT16*)pRawChannel) = (FLOAT32)pInChannel;
		break;
	case 32: *((FLOAT32*)pRawChannel) = (FLOAT32)pInChannel;
		break;
...
VN_STATUS vnPackFixedChannel(INT64 pInChannel, BOOL bSigned, UINT8 uiChannelRate,
                             UINT32 uiSumOffset, UINT64* pRawData) {
	//
	...
	UINT64 uiMaskChannelVal = ((UINT64)1 << uiChannelRate) - 1;
	// bit mask for our destination channel
	...
	if (bSigned) {
		...
		if (pInChannel < 0) { iChannelData = (INT64)iMinChannelVal * pInChannel / VN_MIN_INT32; }
		else { iChannelData = (INT64)iMaxChannelVal * pInChannel / VN_MAX_INT32; }
	}
	else {
...
VN_STATUS vnPackFloatPixel(CONST VN_PIXEL_BLOCK& pSourceBlock, VN_IMAGE_FORMAT format,
                           UINT8* pRawPixel) {
...
VN_STATUS vnPackFixedPixel(CONST VN_PIXEL_BLOCK& pSourceBlock, VN_IMAGE_FORMAT format,
                           UINT8* pRawPixel) {
	//
	...
	for (UINT32 i = 0; i < uiChannelCount; i++) {
		...
		if (VN_FAILED(
			vnPackFixedChannel( pSourceBlock.iChannelData[i], VN_IS_SIGNED_FORMAT( format ), uiChannelRate
				, uiSumOffset, &uiRawData ))) { return vnPostError(VN_ERROR_EXECUTION_FAILURE); }
	...
	if (uiSumOffset) { memcpy(pRawPixel, &uiRawData, (uiSumOffset >> 3)); }
...
VN_STATUS vnConvertFromBlock(CONST VN_PIXEL_BLOCK& pSourceBlock, VN_IMAGE_FORMAT format,
                             UINT8* pRawPixel) {
	if (VN_PARAM_CHECK) {
		if (!pRawPixel || !VN_IS_FORMAT_VALID(format)) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
	...
	VN_IMAGE_PRECISION formatPrecision = bFloatFormat
		                                     ? VN_IMAGE_PRECISION_FLOAT
