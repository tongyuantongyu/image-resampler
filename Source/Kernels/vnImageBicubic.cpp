...
VN_STATUS vnBicubicKernel(CONST CVImage& pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC, FLOAT32 fX,
	...
	for (INT32 j = -2 + 1; j <= 2; j++)
		for (INT32 i = -2 + 1; i <= 2; i++) {
			...
			if (iX < 0 || iY < 0 ||
				...
				iY > pSrcImage.QueryHeight() - 1) { continue; }
...
VN_STATUS vnSampleBicubicVertical(CONST CVImage& pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC,
                                  FLOAT32 fX, FLOAT32 fY, UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
...
VN_STATUS vnSampleBicubicHorizontal(CONST CVImage& pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC,
                                    FLOAT32 fX, FLOAT32 fY, UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
...
VN_STATUS vnBicubicKernel(CONST CVImage& pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC, FLOAT32 fX,
                          FLOAT32 fY, BOOL bDirection, UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
