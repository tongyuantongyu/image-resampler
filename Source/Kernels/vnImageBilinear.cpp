...
VN_STATUS vnLerpBlocks(CONST VN_PIXEL_BLOCK& blockA, CONST VN_PIXEL_BLOCK& blockB, FLOAT32 fDelta,
	...
	if (VN_PARAM_CHECK) { if (!pOutBlock) { return vnPostError(VN_ERROR_INVALIDARG); } }

	if (blockA.uiPrecision != blockB.uiPrecision) { return vnPostError(VN_ERROR_INVALID_RESOURCE); }

	if (VN_IMAGE_PRECISION_FLOAT == blockA.uiPrecision) {
		pOutBlock->fChannelData[0] = blockA.fChannelData[0] * (1.0 - fDelta) + blockB.fChannelData[0] *
			fDelta;
		pOutBlock->fChannelData[1] = blockA.fChannelData[1] * (1.0 - fDelta) + blockB.fChannelData[1] *
			fDelta;
		pOutBlock->fChannelData[2] = blockA.fChannelData[2] * (1.0 - fDelta) + blockB.fChannelData[2] *
			fDelta;
		pOutBlock->fChannelData[3] = blockA.fChannelData[3] * (1.0 - fDelta) + blockB.fChannelData[3] *
			fDelta;
	}
	else {
		pOutBlock->iChannelData[0] = (UINT64)((FLOAT64)blockA.iChannelData[0] * (1.0 - fDelta) + blockB.
			iChannelData[0] * fDelta);
		pOutBlock->iChannelData[1] = (UINT64)((FLOAT64)blockA.iChannelData[1] * (1.0 - fDelta) + blockB.
			iChannelData[1] * fDelta);
		pOutBlock->iChannelData[2] = (UINT64)((FLOAT64)blockA.iChannelData[2] * (1.0 - fDelta) + blockB.
			iChannelData[2] * fDelta);
		pOutBlock->iChannelData[3] = (UINT64)((FLOAT64)blockA.iChannelData[3] * (1.0 - fDelta) + blockB.
			iChannelData[3] * fDelta);
	}
...
VN_STATUS vnSampleBilinearHorizontal(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
                                     UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (fX < 0 || fY < 0 || !VN_IS_IMAGE_VALID(pSrcImage) || !pRawOutput) {
			return vnPostError(VN_ERROR_INVALIDARG);
...
VN_STATUS vnSampleBilinearVertical(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
                                   UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (fX < 0 || fY < 0 || !VN_IS_IMAGE_VALID(pSrcImage) || !pRawOutput) {
			return vnPostError(VN_ERROR_INVALIDARG);
...
VN_STATUS vnBilinearKernel(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY, BOOL bDirection,
                           UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
