...
VN_STATUS vnSampleGaussianVertical(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
...
VN_STATUS vnSampleGaussianHorizontal(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
                                     FLOAT32 fRadius, UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (0 == fRadius || !pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
			return vnPostError(VN_ERROR_INVALIDARG);
