	...
	for (INT32 j = -iRadius + 1; j <= iRadius; j++)
		for (INT32 i = -iRadius + 1; i <= iRadius; i++) {
			...
			if (fRadius >= 1.0) {
				...
				if (fDistance >= 0.5f - fRadius) { fWeight = 1.0f - fDistance; }
				else { return vnConvertFromBlock(gTempBlock, pSrcImage.QueryFormat(), pRawOutput); }
			}
...
VN_STATUS vnSampleCoverageVertical(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
...
VN_STATUS vnSampleCoverageHorizontal(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
                                     FLOAT32 fRadius, UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (0 == fRadius || !pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
			return vnPostError(VN_ERROR_INVALIDARG);
