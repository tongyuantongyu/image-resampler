...
VN_STATUS vnSampleSplineVertical(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
                                 UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
...
VN_STATUS vnSampleSplineHorizontal(CONST CVImage& pSrcImage, FLOAT32 fX, FLOAT32 fY,
                                   UINT8* pRawOutput) {
	if (VN_PARAM_CHECK) {
		if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) { return vnPostError(VN_ERROR_INVALIDARG); }
	}
